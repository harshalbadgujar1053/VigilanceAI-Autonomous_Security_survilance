...
def get_reports(db: Session = Depends(get_db)):
    ...
    alert_ids = [r.alert_id for r in reports]
    alerts_by_id = {}
    if alert_ids:
        for a in db.query(AlertRecord).filter(AlertRecord.id.in_(alert_ids)).all():
            alerts_by_id[a.id] = a
    return {"success": True, "count": len(reports), "reports": [
        {
            ...
            "description": alerts_by_id[r.alert_id].description if r.alert_id in alerts_by_id else "",
